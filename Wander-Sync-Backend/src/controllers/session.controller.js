...
export const getSessions = asyncHandler(async (req, res) => {
  ...
  const pastSessions = await Session.find({
    userId: req.user._id,
    $or: [{ expiresAt: { $lt: now } }, { revokedAt: { $exists: true } }],
  })
    .sort({ lastActiveAt: -1 })
...
export const revokeSession = asyncHandler(async (req, res) => {
  console.log("Revoke session request");
  const { id } = req.params;
  console.log("Id", id);

  const sessionToBeRevoked = await Session.deleteOne({
    _id: id,
    userId: req.user.id,
  });
  console.log("Session To Be Revoked", sessionToBeRevoked);
  ...
  await RevokedToken.findOneAndUpdate(
    { token: sessionToBeRevoked.refreshToken },
    { sessionId: id },
    {
      upsert: true, // "Find or Create" logic
      returnDocument: "after", // Fixes your Mongoose warning
